...
func (r *channelMonitorV2Repository) GetConfig(ctx context.Context) (*service.ChannelMonitorV2Config, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		scanPostgresInt64Array(&cfg.GroupIDs), scanPostgresStringArray(&cfg.IgnoredErrorCategories),
		&thresholds,
...
func (r *channelMonitorV2Repository) UpdateConfig(ctx context.Context, cfg service.ChannelMonitorV2Config, expectedVersion int) (*service.ChannelMonitorV2Config, error) {
	...
	err = r.db.QueryRowContext(ctx, `
		...
		RETURNING version, enabled, refresh_interval_seconds, platforms, group_ids,
		          ...
		          COALESCE(health_thresholds, '{}'::jsonb),
		          updated_at, updated_by`,
		cfg.Enabled, cfg.RefreshIntervalSeconds, platforms, postgresInt64Array(cfg.GroupIDs),
		postgresStringArray(cfg.IgnoredErrorCategories), thresholds, cfg.UpdatedBy, expectedVersion,
	).Scan(&updated.Version, &updated.Enabled, &updated.RefreshIntervalSeconds, &raw,
		scanPostgresInt64Array(&updated.GroupIDs), scanPostgresStringArray(&updated.IgnoredErrorCategories),
		&rawThresholds,
...
func (r *channelMonitorV2Repository) loadChannelMonitorV2GroupInfo(ctx context.Context, groupIDs []int64) (map[int64]channelMonitorV2GroupInfo, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT id, COALESCE(name, ''), lower(COALESCE(NULLIF(TRIM(platform), ''), 'unknown')) FROM groups WHERE id = ANY($1) AND deleted_at IS NULL AND status = 'active'`, postgresInt64Array(groupIDs))
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (r *channelMonitorV2Repository) loadErrorDetails(ctx context.Context, filter service.ChannelMonitorV2Filter, cfg service.ChannelMonitorV2Config) (map[string][]service.ChannelMonitorV2ErrorDetail, error) {
	...
	if len(platforms) > 0 {
		args = append(args, postgresStringArray(platforms))
	...
	if groupScopeEmpty {
		...
		args = append(args, postgresInt64Array(groups))
	...
	query := `SELECT
			...
			LEFT(CONCAT_WS(': ',
				NULLIF(current_error.error_type, ''),
				NULLIF(current_error.provider_error_code, ''),
				NULLIF(current_error.provider_error_type, ''),
				NULLIF(current_error.network_error_type, '')
			), 160) AS message,
			COUNT(*) AS count
...
func (r *channelMonitorV2Repository) loadIgnoredErrorCounts(
	...
	args = append(args, postgresStringArray(cfg.IgnoredErrorCategories), service.ChannelMonitorV2TaxonomyVersion)
	catIdx := len(args) - 1
	taxIdx := len(args)
	query := fmt.Sprintf(
