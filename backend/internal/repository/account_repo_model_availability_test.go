...
func TestListModelAvailabilityCandidates_UngroupedQueryIgnoresTransientState(t *testing.T) {
	...
	accounts, err := repo.ListModelAvailabilityCandidates(context.Background(), nil, []string{service.PlatformOpenAI}, false)
	require.NoError(t, err)
	...
	for _, configuredPredicate := range []string{"status", "schedulable", "platform", "account_groups"} {
		require.Contains(t, whereClause, configuredPredicate)
	}
	for _, transientPredicate := range []string{"rate_limit_reset_at", "overload_until", "temp_unschedulable_until", "expires_at", "auto_pause_on_expired"} {
...
func TestListModelAvailabilityCandidates_GroupQueryIgnoresTransientState(t *testing.T) {
	...
	mock.ExpectQuery("model availability candidates").
		WillReturnRows(sqlmock.NewRows([]string{"id"}))
	groupID := int64(42)
	accounts, err := repo.ListModelAvailabilityCandidates(context.Background(), &groupID, []string{service.PlatformAnthropic}, false)
	require.NoError(t, err)
	require.Empty(t, accounts)
	require.NoError(t, mock.ExpectationsWereMet())
