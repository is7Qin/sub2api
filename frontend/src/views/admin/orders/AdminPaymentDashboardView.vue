...
function sortedTopUsers(users: Record<string, TopUserPaymentStats[]>): [string, TopUserPaymentStats[]][] {
  return Object.entries(users || {}).sort(([left], [right]) => left.localeCompare(right))
}

function hasTopUsers(users: Record<string, TopUserPaymentStats[]>): boolean {
  return Object.values(users || {}).some(rows => rows.length > 0)
}
