      ...
      >
        ...
        >
          ...
          <div class="flex items-center justify-between mt-4">
            <label class="text-sm text-gray-600 dark:text-gray-400">{{
              t("admin.groups.openaiMessages.longContextBilling")
            }}</label>
            <button
              type="button"
              @click="
                createForm.openai_long_context_billing_enabled =
                  !createForm.openai_long_context_billing_enabled
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {{ t("admin.groups.openaiMessages.longContextBillingHint") }}
          </p>
        ...
        >
          ...
          <div class="flex items-center justify-between mt-4">
            ...
            <button
              type="button"
              @click="
                editForm.openai_long_context_billing_enabled =
                  !editForm.openai_long_context_billing_enabled
              "
              class="relative inline-flex h-6 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
              :class="
...
const buildWebSearchFinalPricePreview = (form: {
  ...
  // 留空 = 默认价 0.01；显式 0 = 免费（需保留 0，不能用 fallback 吞掉）
  const raw = form.web_search_price_per_call;
  const basePrice =
    raw === "" || raw === null || raw === undefined
      ? DEFAULT_WEB_SEARCH_PRICE_PER_CALL
      : normalizePreviewNumber(raw, DEFAULT_WEB_SEARCH_PRICE_PER_CALL);
...
const closeCreateModal = () => {
  ...
  createForm.openai_long_context_billing_enabled = false;
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.openai_long_context_billing_enabled =
    group.openai_long_context_billing_enabled ?? false;
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    // 清空输入 = 清除回默认价 0.01（后端负数语义），与图片价格口径一致
    payload.web_search_price_per_call =
      (payload.web_search_price_per_call as number | string | null) === ""
        ? -1
        : payload.web_search_price_per_call;
