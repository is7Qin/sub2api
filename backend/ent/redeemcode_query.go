...
func (_q *RedeemCodeQuery) QueryLotteryDraws() *LotteryDrawQuery {
...
func (_q *RedeemCodeQuery) WithLotteryDraws(opts ...func(*LotteryDrawQuery)) *RedeemCodeQuery {
	query := (&LotteryDrawClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RedeemCodeQuery) loadLotteryDraws(ctx context.Context, query *LotteryDrawQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *LotteryDraw)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(lotterydraw.FieldRedeemCodeID)
	}
	query.Where(predicate.LotteryDraw(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(redeemcode.LotteryDrawsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
