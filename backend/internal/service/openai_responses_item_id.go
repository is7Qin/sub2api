...
// sanitizeOpenAIResponsesInputItemIDs rebuilds the top-level input array once,
// avoiding repeated whole-body deletes when a request contains many invalid IDs.
func sanitizeOpenAIResponsesInputItemIDs(body []byte) ([]byte, bool, error) {
	if !gjson.ValidBytes(body) {
		return body, false, fmt.Errorf("invalid JSON request body")
	}
	...
	input.ForEach(func(_, item gjson.Result) bool {
		...
		if item.IsObject() {
			...
			if itemType.Type == gjson.String && id.Type == gjson.String &&
				...
				if sanitizeErr != nil {
					sanitizeErr = fmt.Errorf("delete Responses input item id: %w", sanitizeErr)
					return false
				}
				changed = true
			}
