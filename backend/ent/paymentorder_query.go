...
func (_q *PaymentOrderQuery) QueryRechargeResetRecords() *RechargeResetRecordQuery {
...
func (_q *PaymentOrderQuery) WithRechargeResetRecords(opts ...func(*RechargeResetRecordQuery)) *PaymentOrderQuery {
	query := (&RechargeResetRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withRechargeResetRecords; query != nil {
		if err := _q.loadRechargeResetRecords(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.RechargeResetRecords = []*RechargeResetRecord{} },
			func(n *PaymentOrder, e *RechargeResetRecord) {
...
func (_q *PaymentOrderQuery) loadRechargeResetRecords(ctx context.Context, query *RechargeResetRecordQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *RechargeResetRecord)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rechargeresetrecord.FieldOrderID)
	}
	query.Where(predicate.RechargeResetRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.RechargeResetRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
