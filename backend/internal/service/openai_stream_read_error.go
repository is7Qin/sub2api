...
// OpenAIUpstreamStreamReadErrorDetails exposes only a stable client
// classification; raw transport details remain available through Unwrap for logs.
...
func classifyOpenAIUpstreamStreamReadError(err error) (code, message string) {
	if err != nil {
		...
		// net/http's HTTP/2 stream error is unexported. Match its transport
		// signature, but never expose the original stream ID or reset reason.
