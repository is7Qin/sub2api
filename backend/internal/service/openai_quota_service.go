...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	accessToken, chatGPTAccountID, proxyURL, isFedRAMP, err := s.prepareUpstreamCall(ctx, accountID)
	if err != nil {
		return nil, err
	}
	...
	resp, err := client.R().
		SetContext(callCtx).
		SetHeaders(buildCodexCommonHeaders(accessToken, chatGPTAccountID, isFedRAMP)).
...
func (s *OpenAIQuotaService) ResetCredit(ctx context.Context, accountID int64) (*OpenAIQuotaResetResult, error) {
	...
	headers := buildCodexCommonHeaders(accessToken, chatGPTAccountID, isFedRAMP)
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (accessToken, chatGPTAccountID, proxyURL string, isFedRAMP bool, err error) {
	...
	if !account.IsOpenAIOAuthLike() {
		return "", "", "", false, infraerrors.New(http.StatusBadRequest, "OPENAI_QUOTA_INVALID_TYPE", "account is not an OpenAI OAuth-like account")
	}
	...
	return accessToken, chatGPTAccountID, proxyURL, account.IsOpenAIChatGPTFedRAMPAccount(), nil
}
