...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		accountRelease, acquired := h.acquireResponsesAccountSlot(c, selection, false, &streamStarted, reqLog)
		if !acquired {
			return
		}
		...
		if err == nil {
			...
			if result != nil {
				h.recordAlphaSearchUsage(c, apiKey, account, subscription, channelMapping, requestedModel, forwardBody, result, subject.UserID)
			}
			return
		}
