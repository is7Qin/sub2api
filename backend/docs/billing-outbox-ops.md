...
| 条件 | 阈值 | 推演 |
| --- | --- | --- |
| terminal 绝对量 | > 500,000 行 | 健康稳态终态化预算 ≤ 0.1 行/s（10K/s 摄入的 0.001%）；30 天保留窗口稳态堆积 ≈ 26 万行，阈值 ≈ 2× 该上限——健康累计不误报，超阈说明终态化率长期偏离预算或清理停摆 |
...
- 触发沿（条件从正常越界）记一条 `slog.Error`（含 terminal 行数、阈值、速率）并置位
  `health.terminal_alert`；**同一条件在 30 分钟冷却期内不重复刷屏**，持续条件冷却后
  重新确认。
- 条件回落（计数低于阈值且速率归零）自动清除 `terminal_alert` 并重新武装；清理删除
  产生负 delta，不会误报。
