  ...
  admin: {
    ...
    channelMonitor: {
      ...
      form: {
        ...
        jitterSecondsHint: '每轮调度在 ± 抖动秒数内随机偏移。范围：0 - {max} 秒。',
        enabled: '启用监控',
        kindRequired: '请选择供应商'
      },
      runResultTitle: '检测结果',
    ...
    subscriptions: {
      ...
      switchSubscriptionGroup: '切换订阅分组',
      ...
      currentGroup: '当前分组',
      switchingFor: '为以下用户切换订阅',
      switchGroupHint: '切换分组时会保留已使用的用量信息',
      ...
      guide: {
        ...
        actions: {
          ...
          switchGroup: '切换',
          switchGroupDesc: '切换订阅分组并保留已使用用量',
          resetQuota: '重置配额',
    ...
    accounts: {
      ...
      oauth: {
        ...
        openai: {
          ...
          personalAccessTokenAuth: '手动输入 PAT',
          personalAccessTokenDesc: '输入 Codex Personal Access Token，系统将按第一步配置创建 OpenAI OAuth-like 账号。',
          personalAccessTokenInputLabel: 'Personal Access Token',
          personalAccessTokenPlaceholder: '粘贴 personal_access_token / PAT...',
          personalAccessTokenHint: 'PAT 将保存为 personal_access_token，不要求 accessToken 过期时间，也不会触发 OAuth refresh。',
          personalAccessTokenImportAndCreate: '使用 PAT 创建账号',
          personalAccessTokenEmpty: '请输入 Personal Access Token',
          ...
          codexSessionPlaceholder: '支持多行，每行一个 token 或 JSON；PAT 请使用 personal_access_token 字段',
          codexSessionHint: 'sessionToken 不会作为 refresh_token 保存；personal_access_token 不要求 accessToken 过期时间，导入数据缺少该字段时会忽略；仅 accessToken 且无 refresh_token 时仍需可解析过期时间或在第一步设置过期时间。',
          codexSessionImportAndCreate: '导入并创建账号',
          codexSessionEmpty: '请输入 Codex JSON、accessToken 或 personal_access_token JSON',
    ...
    ops: {
      ...
      diagnosis: {
        ...
        circuitBreakerOpen: '计费熔断已打开',
        circuitBreakerOpenImpact: '计费 apply Claim 已暂停，直至冷却试探恢复；新计费写入仍正常入队。',
        circuitBreakerOpenAction: '查看计费 worker 卡片的 last_error 与 outbox 运维文档的恢复指引。',
        terminalAlert: '计费 terminal 行数增长超阈值',
        terminalAlertImpact: '系统性永久错误正在被 terminal 化（如 schema 损坏）。',
        terminalAlertAction: '检查 billing-outbox/health 的 stats_error 与 terminal 恢复 SQL。',
        billingLagCritical: '计费最长延迟已超过 {lag} 小时',
        billingLagCriticalImpact: '队列中的记录扣费被推迟。',
        billingLagCriticalAction: '观察计费 worker 卡片的积压数；延迟 ≈ 积压数 / 消化速率。',
    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexUserAgent: 'OpenAI Codex UA 配置',
        openaiCodexUserAgentHint: '用于新建 OpenAI OAuth/Codex 账号指纹的结构化 UA 配置。已有账号继续使用已持久化的指纹 profile，直到正常重新授权或新建指纹时更新。',
        openaiCodexOriginator: '来源标识',
        openaiCodexOriginatorPlaceholder: 'codex-tui',
        openaiCodexVersion: 'Codex 版本',
        openaiCodexVersionPlaceholder: '0.136.0',
        openaiCodexOS: '系统指纹',
        openaiCodexOSPlaceholder: 'Mac OS 26.5.0; arm64',
        openaiCodexTerminal: '终端标识',
        openaiCodexTerminalPlaceholder: 'Apple_Terminal/470.2',
        openaiCodexUserAgentPreview: '生效 / 生成的 User-Agent',
      ...
      openai403Cooldown: {
        ...
        thresholdPauseSeconds: '阈值暂停时长（秒）',
      ...
      openaiOAuth429Dynamic: {
        ...
        planTypeRequired: 'Plan Type 不能为空',
        planTypeDuplicate: 'Plan Type 不能重复',
        planTypeLimit: 'Plan Type 策略最多允许 100 项',
        enabled: '启用动态调度',
        ...
        usageWindowCheckHint: '仅在原有 429 规则触发后生效；已知 5 小时或 7 天窗口达到阈值时才暂停。两个窗口都缺失时，还需账号创建时间超过下方等待时长。',
        usageWindow5hThreshold: '5 小时用量阈值（%）',
        usageWindow7dThreshold: '7 天用量阈值（%）',
        usageWindowMissingDataFallbackSeconds: '缺失用量回退等待（秒）',
        usageWindowMissingDataFallbackSecondsHint: '账号创建后超过此时长且 5 小时、7 天用量仍全部缺失，才沿用原有 429 暂停逻辑。0 表示创建后即可回退。',
