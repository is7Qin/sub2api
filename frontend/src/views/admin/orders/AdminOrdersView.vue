        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ formatProductAmount(row, row.refund_amount) }}</span>
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatProductAmount(selectedOrder, selectedOrder.amount) }}</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ formatProductAmount(selectedOrder, selectedOrder.refund_amount) }}</p></div>
...
async function handleRefund(data: { amount: number; reason: string; deduct_balance: boolean; force: boolean }) {
  ...
  try {
    const response = await adminPaymentAPI.refundOrder(selectedOrder.value.id, { amount: data.amount, reason: data.reason, deduct_balance: data.deduct_balance, force: data.force })
    if (response.data.require_force) {
      // A failed preparation may require explicit approval for a partial balance
      // deduction; preserve the entered refund and surface that choice in-place.
      refundRequireForce.value = true
      refundWarning.value = response.data.warning || ''
      return
    }
