...
func (s *openAISnapshotCacheStub) GetSchedulableAccountsByIDs(_ context.Context, ids []int64) (map[int64]*Account, error) {
	accounts := make(map[int64]*Account, len(ids))
	for _, id := range ids {
		account := s.accountsByID[id]
		if account == nil {
			for _, candidate := range s.snapshotAccounts {
...
func newPublishedOpenAISnapshotCache(accounts []Account) *openAISnapshotCacheStub {
	...
	for i := range accounts {
		account := accounts[i]
		cache.snapshotAccounts = append(cache.snapshotAccounts, &account)
		cache.accountsByID[account.ID] = &account
	}
	return cache
}

type noBatchOpenAISnapshotCache struct {
	SchedulerCache
...
func (c noBatchOpenAISnapshotCache) GetAccount(ctx context.Context, accountID int64) (*Account, error) {
	return c.cache.GetAccount(ctx, accountID)
}
...
type schedulerModelAvailabilityOpenAIAccountRepo struct {
	schedulerTestOpenAIAccountRepo
	listModelAvailabilityCandidates func(context.Context, *int64, []string, bool) ([]Account, error)
}

func (r schedulerModelAvailabilityOpenAIAccountRepo) ListModelAvailabilityCandidates(ctx context.Context, groupID *int64, platforms []string, includeGrouped bool) ([]Account, error) {
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_Enabled_PureUnsupportedModelReturnsTypedError(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
		WithPublicModelSupportMiss404(ctx),
		&groupID,
		"",
		"",
		"gpt-unknown-public",
	...
	require.True(t, ok)
	require.Equal(t, "gpt-unknown-public", model)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_Enabled_TransientSupportingAccountStaysRetryable(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	repo := schedulerModelAvailabilityOpenAIAccountRepo{
		schedulerTestOpenAIAccountRepo: schedulerTestOpenAIAccountRepo{accounts: nil},
	...
	require.Equal(t, []string{PlatformOpenAI}, requestedPlatforms)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_Enabled_AvailabilityLookupFailureStaysRetryable(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyRateLimitedAccountFallsBackToFreshCandidate(t *testing.T) {
	...
	snapshotCache := &openAISnapshotCacheStub{snapshotAccounts: []*Account{freshSticky, freshBackup}, accountsByID: map[int64]*Account{31001: freshSticky, 31002: freshBackup}}
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_SkipsFreshlyRateLimitedSnapshotCandidate(t *testing.T) {
	...
	snapshotCache := &openAISnapshotCacheStub{snapshotAccounts: []*Account{freshPrimary, freshSecondary}, accountsByID: map[int64]*Account{32001: freshPrimary, 32002: freshSecondary}}
	snapshotService := &SchedulerSnapshotService{cache: snapshotCache}
	svc := &OpenAIGatewayService{
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyPublishedRuntimeRecheckSkipsRateLimitedAccount(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10103)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	...
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{&dbSticky, &dbBackup},
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_PublishedRuntimeRecheckSkipsRateLimitedCandidate(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10104)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	...
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{&dbPrimary, &dbSecondary},
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_PreviousResponseClearsAccountOutsideGroup(t *testing.T) {
	ctx := context.Background()
	groupID := int64(901)
	otherGroupID := int64(902)
	outsideGroup := Account{
		ID:          10001,
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
	...
	cfg.Gateway.Scheduling.LoadBatchEnabled = false
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerGroupAwareOpenAIAccountRepo{schedulerTestOpenAIAccountRepo{accounts: []Account{outsideGroup, insideGroup}}},
		cache:              cache,
		cfg:                cfg,
		rateLimitService:   newOpenAIAdvancedSchedulerRateLimitService("true"),
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{}),
	...
	store := svc.getOpenAIWSStateStore()
	require.NoError(t, store.BindResponseAccount(ctx, groupID, "resp_outside_group", outsideGroup.ID, time.Hour))
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, insideGroup.ID, selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickyPreviousHit)
	boundAccountID, err := store.GetResponseAccount(ctx, groupID, "resp_outside_group")
	require.NoError(t, err)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_ClearsStickyAccountOutsideGroupBeforeEligibility(t *testing.T) {
	ctx := context.Background()
	groupID := int64(1014)
	accounts := []Account{
		{
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{
			"openai:session_hash_removed_group_unsupported": 2501,
		},
	}
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		...
		&groupID,
		"",
		"session_hash_removed_group_unsupported",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		false,
	...
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.False(t, decision.StickySessionHit)
	require.Equal(t, 1, cache.deletedSessions["openai:session_hash_removed_group_unsupported"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_SimpleModeAllowsGroupedStickyWithoutGroup(t *testing.T) {
	ctx := context.Background()
	account := Account{
		ID:          2601,
		Platform:    PlatformOpenAI,
	...
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{
			"openai:session_hash_simple_grouped": account.ID,
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: []Account{account}},
	...
	require.Equal(t, openAIAccountScheduleLayerSessionSticky, decision.Layer)
	require.True(t, decision.StickySessionHit)
	require.Zero(t, cache.deletedSessions["openai:session_hash_simple_grouped"])
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}
...
func TestDefaultOpenAIAccountScheduler_SetupTokenBypassesRequirePrivacySetWithoutClaimingPrivacy(t *testing.T) {
	...
	snapshot := NewSchedulerSnapshotService(newPublishedOpenAISnapshotCache(repo.accounts), nil, repo, openAIPrivacySchedulerGroupRepo{group: &Group{
		ID:                groupID,
...
func TestDefaultOpenAIAccountScheduler_RequirePrivacySetUsesPublishedAccountBeforeBlockingStaleSnapshot(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	...
	snapshotCache := &openAISnapshotCacheStub{
		...
		accountsByID: map[int64]*Account{
			freshFullOAuth.ID: &freshFullOAuth,
