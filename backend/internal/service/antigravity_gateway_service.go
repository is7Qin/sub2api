...
// resolveAntigravityForwardBaseURL defaults production OAuth traffic to the
// production endpoint. The daily sandbox is only selected explicitly.
...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			// 网络错误时，继续重试；一旦请求已 admitted，旧 HTTP 响应便不再代表最新结果。
			if retryErr != nil || retryResp == nil {
				if IsHTTPUpstreamAttemptNotAdmitted(retryErr) || isHTTPUpstreamRetryNotAdmitted(retryErr) {
					if lastRetryErr != nil {
						return &smartRetryResult{action: smartRetryActionBreakWithResp, err: lastRetryErr}
					}
				...
				networkErr := retryErr
				if networkErr == nil {
					networkErr = errors.New("upstream returned nil response")
				} else {
					lastRetryErr = retryErr
				}
				log.Printf("%s status=smart_retry_network_error attempt=%d/%d error=%v", p.prefix, attempt, maxAttempts, networkErr)
				continue
			}
		...
		// 真实传输错误是最新 admitted 结果，不能用更早的 HTTP 响应覆盖。
		if lastRetryErr != nil {
			return &smartRetryResult{action: smartRetryActionBreakWithResp, err: lastRetryErr}
		}
...
func (s *AntigravityGatewayService) handleSingleAccountRetryInPlace(
	...
	for attempt := 1; attempt <= antigravitySingleAccountSmartRetryMaxAttempts; attempt++ {
		...
		// 网络错误时继续重试；一旦请求已 admitted，旧 503 便不再代表最新结果。
		if retryErr != nil || retryResp == nil {
			if IsHTTPUpstreamAttemptNotAdmitted(retryErr) || isHTTPUpstreamRetryNotAdmitted(retryErr) {
				if lastRetryErr != nil {
					return &smartRetryResult{action: smartRetryActionBreakWithResp, err: lastRetryErr}
			...
			logger.LegacyPrintf("service.antigravity_gateway", "%s single_account_503_retry: network_error attempt=%d/%d error=%v",
				p.prefix, attempt, antigravitySingleAccountSmartRetryMaxAttempts, networkErr)
			continue
		}
	...
	// 真实传输错误是最新 admitted 结果，不能用更早的 503 覆盖。
	if lastRetryErr != nil {
		return &smartRetryResult{action: smartRetryActionBreakWithResp, err: lastRetryErr}
	}
...
urlFallbackLoop:
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			if err != nil {
				if _, notAdmitted := completedError.ifRetryNotAdmitted(err); notAdmitted {
					if lastTransportErr != nil {
						return nil, lastTransportErr
					}
...
func (s *AntigravityGatewayService) checkErrorPolicy(ctx context.Context, account *Account, statusCode int, body []byte, requestedModel ...string) ErrorPolicyResult {
	...
	return s.rateLimitService.CheckErrorPolicy(ctx, account, statusCode, body, requestedModel...)
}
...
func (s *AntigravityGatewayService) handleUpstreamError(
	...
	shouldDisable := s.rateLimitService.HandleUpstreamError(ctx, account, statusCode, headers, body, resolveFinalAntigravityModelKey(ctx, account, requestedModel))
	if shouldDisable {
...
func (s *AntigravityGatewayService) handleClaudeStreamToNonStreaming(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string) (*antigravityStreamResult, error) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			last = parsed
			if parsedUsage := extractGeminiUsage(inner); parsedUsage != nil {
				usage = parsedUsage
			}
...
returnResponse:
	...
	// TransformGeminiToClaude is the terminal conversion source for a complete response.
	usage = &ClaudeUsage{
