...
function persistTokenPair(tokens: RefreshTokenResponse): void {
  ...
  // Write the rotating token last so other tabs can treat its change as the commit marker.
...
async function requestTokenPair(
  ...
  // A losing request may receive 401 before the winning peer publishes its rotated token.
...
/**
 * Calls in one document share a promise; Web Locks serialize token rotation across tabs.
 */
