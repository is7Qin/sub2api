...
func computeBasicStats(st *DashboardStats, orders []*dbent.PaymentOrder, todayStart time.Time) {
	...
	for currency, amount := range st.TotalAmount {
		st.AvgAmount[currency] = roundPaymentAmount(currency, amount/float64(currencyCounts[currency]))
	}
...
func buildTopUsers(orders []*dbent.PaymentOrder) TopUsersByCurrency {
	...
	for _, o := range orders {
		currency := PaymentOrderCurrency(o)
		if userMap[currency] == nil {
			userMap[currency] = make(map[int64]*TopUserStat)
		}
		us, ok := userMap[currency][o.UserID]
		if !ok {
			us = &TopUserStat{UserID: o.UserID, Email: o.UserEmail}
			userMap[currency][o.UserID] = us
		}
	...
	for currency, users := range userMap {
		...
		for _, us := range users {
			us.Amount = roundPaymentAmount(currency, us.Amount)
...
func roundCurrencyAmounts(amounts CurrencyAmounts) {
	for currency, amount := range amounts {
		amounts[currency] = roundPaymentAmount(currency, amount)
	}
}

func roundPaymentAmount(currency string, amount float64) float64 {
