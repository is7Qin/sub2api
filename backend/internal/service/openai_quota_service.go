...
func (s *OpenAIQuotaService) QueryUsage(ctx context.Context, accountID int64) (*OpenAIQuotaUsage, error) {
	account, accessToken, chatGPTAccountID, proxyURL, isFedRAMP, err := s.prepareUpstreamCall(ctx, accountID)
	if err != nil {
		return nil, err
	}
	...
	doRequest := func(authValue string) (*req.Response, error) {
		payload = OpenAIQuotaUsage{}
		return client.R().
			SetContext(callCtx).
			SetHeaders(buildCodexCommonHeaders(authValue, chatGPTAccountID, isFedRAMP)).
	...
	if err == nil && account.IsOpenAIAgentIdentity() && !resp.IsSuccessState() && isAgentIdentityTaskInvalidHTTPResponse(resp.StatusCode, resp.Bytes()) {
		...
		resp, err = doRequest(headers.Get("Authorization"))
	}
	if err != nil {
	...
	if !resp.IsSuccessState() {
		...
		body := truncate(string(redactAgentIdentitySensitiveBodyForAccount(ctx, s.accountRepo, account, resp.Bytes())), 240)
...
func (s *OpenAIQuotaService) ResetCredit(ctx context.Context, accountID int64) (*OpenAIQuotaResetResult, error) {
	...
	doRequest := func(authValue string) (*req.Response, error) {
		...
		headers := buildCodexCommonHeaders(authValue, chatGPTAccountID, isFedRAMP)
...
func (s *OpenAIQuotaService) prepareUpstreamCall(ctx context.Context, accountID int64) (account *Account, accessToken, chatGPTAccountID, proxyURL string, isFedRAMP bool, err error) {
	...
	if !account.IsOpenAIOAuthLike() {
		return nil, "", "", "", false, infraerrors.New(http.StatusBadRequest, "OPENAI_QUOTA_INVALID_TYPE", "account is not an OpenAI OAuth-like account")
	}
	...
	return account, accessToken, chatGPTAccountID, proxyURL, account.IsOpenAIChatGPTFedRAMPAccount(), nil
}
