...
func TestSystemHandlerPerformUpdateSurvivesClientDisconnect(t *testing.T) {
	...
	require.Equal(t, 1, updateSvc.performCall)
	require.NoError(t, updateSvc.performCtxErr, "update must not observe the canceled request context")
	require.True(t, updateSvc.performHasDeadline, "detached update context must still be bounded")
	requireSystemLockStatus(t, repo, service.IdempotencyStatusSucceeded)
}
...
func TestSystemHandlerRollbackToVersionSurvivesClientDisconnect(t *testing.T) {
	...
	require.NoError(t, updateSvc.rollbackToCtxErr, "versioned rollback must not observe the canceled request context")
	require.True(t, updateSvc.rollbackToHasDeadline, "detached rollback context must still be bounded")
	requireSystemLockStatus(t, repo, service.IdempotencyStatusSucceeded)
}
